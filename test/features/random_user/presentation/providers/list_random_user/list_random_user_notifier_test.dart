...
void main() {
  ...
  late ListRandomUsersNotifier listRandomUsersNotifier;
  late GetListOfRandomUsers getListOfRandomUsers;
  ...
  setUpAll(() async {
    ...
    getListOfRandomUsers = GetListOfRandomUsers(repository);
    listRandomUsersNotifier = ListRandomUsersNotifier(getListOfRandomUsers);
  });

  stateNotifierTest<ListRandomUsersNotifier, ListRandomUsersState>(
    'Should fetch a list of randomUsers',
    build: () => listRandomUsersNotifier,
    setUp: () {
      when(() => connectivity.checkConnectivity()).thenAnswer(
        (_) async => ConnectivityResult.mobile,
      );
    ...
    actions: (notifier) async {
      await notifier.fetchListRandomUsers(3);
    },
    expect: () => [
      isInstanceOf<ListRandomUsersState>(),
      isInstanceOf<ListRandomUsersState>(),
    ],
  );
}
